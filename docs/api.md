...
| `activityConfidenceThreshold` | `Number`      | Android, iOS | **Since 4.5.2.** 0-100. Ignore STILL/ACTIVE transitions below this confidence (prevents jittery GPS start/stop). iOS normalizes CMMotionActivityConfidence (Low/Med/High → 20/40/80).                                                                                                                                                                | ACT         | `50`                       |
| `maxAcceptedAccuracy`     | `Number`          | Android, iOS | **Since 4.5.2.** Maximum accepted horizontal accuracy in meters. Fixes worse than this are dropped before persist/POST/JS emission. **Off by default (`null`)** to avoid regressing existing apps. Recommended: `100` para tracking vehicular, `500` para tolerante.                                                                                | all         | `null`                     |
